...
def handle_webrtc_audio(key):
    ...
    st.session_state[f"recording_status_{key}"] = st.session_state.get(f"recording_status_{key}", "Not recording")

    def process_audio_frame(audio_frame: av.AudioFrame) -> av.AudioFrame:
        try:
            audio_data = audio_frame.to_ndarray().flatten()
            audio_queue.put(audio_data)
            logger.debug(f"Received audio frame for {key}: {len(audio_data)} samples")
        except Exception as e:
            logger.error(f"Error processing audio frame for {key}: {str(e)}")
        return audio_frame
    ...
    try:
        ...
        logger.error(f"Failed to initialize webrtc_streamer for {key}: {str(e)}")
        st.error(f"WebRTC initialization failed: {str(e)}. Please check your network or try a different browser.")
        return None
    ...
    elif st.session_state[f"recording_status_{key}"] == "Recording...":
    ...
    if ctx and not ctx.state.playing and st.session_state[f"recording_status_{key}"] == "Stopped":
        try:
            audio_frames = []
            timeout = 15
            start_time = time.time()
            while time.time() - start_time < timeout:
                try:
                    ...
                    if audio_data is not None:
                        ...
                        logger.debug(f"Collected audio frame for {key}, total frames: {len(audio_frames)}")
                except queue.Empty:
                    logger.debug(f"Audio queue empty for {key}, stopping collection")
                    break
            if audio_frames:
                logger.info(f"Collected {len(audio_frames)} audio frames for {key}")
                audio_data = np.concatenate(audio_frames, axis=0)
                # Save to temporary file and store bytes
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmpfile:
                ...
                st.session_state[f"recorded_audio_{key}"] = audio_bytes
                logger.info(f"Audio saved to st.session_state.recorded_audio_{key} for {key}")
                st.success(f"Audio recorded and saved for {key}")
                if os.path.exists(tmpfile_path):
                    os.remove(tmpfile_path)
                return audio_bytes
            else:
                logger.warning(f"No audio frames captured for {key}")
                st.warning(f"No audio data captured for {key}. Ensure your microphone is enabled and try again.")
                return None
        except Exception as e:
            logger.error(f"Error in WebRTC audio processing for {key}: {str(e)}")
            st.error(f"Error capturing audio for {key}: {str(e)}")
            return None
    return None
...
def main():
    ...
    default_state = {
        ...
        'recorded_audio_audio_en_hi': None,
        'recorded_audio_audio_hi': None
    }
    ...
    with tab1:
        ...
        handle_webrtc_audio("audio_en_hi")
        if st.session_state.get("recorded_audio_audio_en_hi"):
            st.audio(st.session_state.recorded_audio_audio_en_hi, format="audio/wav")
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmpfile:
                tmpfile.write(st.session_state.recorded_audio_audio_en_hi)
        ...
        if st.button("Transcribe with Speech2Text", key="transcribe_en_hi"):
            audio = st.session_state.get("recorded_audio_audio_en_hi") or uploaded_file_en_hi
            logger.info(f"Transcribe button clicked for English/Hindi, audio available: {audio is not None}")
            if audio:
                ...
                st.session_state.recorded_audio_audio_en_hi = None
                logger.info("Transcription completed for English/Hindi")
            else:
    ...
    with tab2:
        ...
        st.markdown("### Record Audio", unsafe_allow_html=True)
        st.info("Click 'Start' to begin recording and 'Stop' to finish. Ensure your microphone is enabled.")
        handle_webrtc_audio("audio_hi")
        if st.session_state.get("recorded_audio_audio_hi"):
            st.audio(st.session_state.recorded_audio_audio_hi, format="audio/wav")
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmpfile:
                tmpfile.write(st.session_state.recorded_audio_audio_hi)
                y, sr = sf.read(tmpfile.name)
                rms = np.sqrt(np.mean(y**2))
                st.write(f"Recorded audio RMS amplitude: {rms:.6f}")
                if rms < 1e-4:
        ...
        if st.button("Transcribe with Speech2Text", key="transcribe_hi"):
            audio = st.session_state.get("recorded_audio_audio_hi") or uploaded_file_hi
            logger.info(f"Transcribe button clicked for Hindi-only, audio available: {audio is not None}")
            if audio:
                ...
                st.session_state.recorded_audio_audio_hi = None
                logger.info("Transcription completed for Hindi-only")
            else:
